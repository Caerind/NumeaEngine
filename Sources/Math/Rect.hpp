...
{
	Vector2<T> max = getMaximum();
	return nu::inRange(x, mMin.x, max.x) && nu::inRange(y, mMin.y, max.y);
}
...
{
	Vector2<T> max = getMaximum();
	return nu::inRange(point.x, mMin.x, max.x) && nu::inRange(point.y, mMin.y, max.y);
}
...
inline bool Rect<T>::intersects(const Rect<T>& rect, Rect<T>* intersection) const
{
	...
	T left = std::max(mMin.x, rect.mMin.x);
	T right = std::min(max.x, max2.x);
	if (left >= right)
		return false;

	T top = std::max(mMin.y, rect.mMin.y);
	T bottom = std::min(max.y, max2.y);
	if (top >= bottom)
		return false;
