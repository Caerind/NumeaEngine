...
{
	const I32 X = I32(nu::floor(x)) & 255;
	const I32 Y = I32(nu::floor(y)) & 255;
	const I32 Z = I32(nu::floor(z)) & 255;

	x -= nu::floor(x);
	y -= nu::floor(y);
	z -= nu::floor(z);

	const F32 u = nu::fade(x);
	const F32 v = nu::fade(y);
	const F32 w = nu::fade(z);
