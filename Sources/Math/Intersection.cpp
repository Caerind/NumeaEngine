...
bool intersects(const AABB & a1, const AABB & a2, AABB * intersection)
{
	...
	F32 left = std::max(min1.x, min2.x);
	F32 right = std::min(max1.x, max2.x);

	if (left >= right)
		return false;

	F32 top = std::max(min1.y, min2.y);
	F32 bottom = std::min(max1.y, max2.y);
	if (top >= bottom)
		return false;

	F32 up = std::max(min1.z, min2.z);
	F32 down = std::min(max1.z, max2.z);
	if (up >= down)
		return false;
...
bool intersects(const Ray & a1, const Ray & a2, F32 * l1, F32 * l2)
{
	...
	if (equals(q2, 0.0f) || equals(d1.x, 0.0f))
	{
		if (l1)
		{
			*l1 = 0.0f;
	...
	if (equals(d1.z * t1 + o1.z - (d2.z * t2 + o2.z), 0.0f))
	{
		if (l1)
		{
			*l1 = t1;
