...
{
	r = U8(nu::clamp(color.r, 0.0f, 1.0f) * 255.0f);
	g = U8(nu::clamp(color.g, 0.0f, 1.0f) * 255.0f);
	b = U8(nu::clamp(color.b, 0.0f, 1.0f) * 255.0f);
	a = U8(nu::clamp(color.a, 0.0f, 1.0f) * 255.0f);
	return *this;
}
...
{
	return Color(U8(std::min(I32(left.r) + right.r, 255)),
		U8(std::min(I32(left.g) + right.g, 255)),
		U8(std::min(I32(left.b) + right.b, 255)),
		U8(std::min(I32(left.a) + right.a, 255)));
}

Color operator-(const Color& left, const Color& right)
{
	return Color(U8(std::max(I32(left.r) - right.r, 0)),
		U8(std::max(I32(left.g) + right.g, 0)),
		U8(std::max(I32(left.b) + right.b, 0)),
		U8(std::max(I32(left.a) + right.a, 0)));
}
