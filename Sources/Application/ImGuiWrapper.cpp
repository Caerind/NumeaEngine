...
float mTime = 0.0f;
bool mMouseJustPressed[3] = { false, false, false };
...
bool init(Window& window)
{
	...
	io.SetClipboardTextFn = priv::setClipboardText;
	io.GetClipboardTextFn = priv::getClipboardText;
	io.ClipboardUserData = priv::mWindow;
	#ifdef _WIN32
	io.ImeWindowHandle = priv::mWindow->getWindowHandle();
	#endif
...
void newFrame()
{
	if (!priv::mFontTexture)
		priv::createDeviceObjects();

	ImGuiIO& io = ImGui::GetIO();

	// Setup display size (every frame to accommodate for window resizing)
	Vector2u ws = priv::mWindow->getSize();
	Vector2u fs = priv::mWindow->getFramebufferSize();
	...
	io.DeltaTime = priv::mTime > 0.0 ? current_time - priv::mTime : (1.0f / 60.0f);
	priv::mTime = current_time;

	// Setup inputs
	// (we already got mouse wheel, keyboard keys & characters from glfw callbacks polled in glfwPollEvents())
	if (priv::mWindow->hasFocus())
	{
		Vector2f m = priv::mWindow->getCursorPos();
		io.MousePos = ImVec2(m.x, m.y); // Get mouse position in screen coordinates (set to -1,-1 if no mouse / on another screen, etc.)
	}
	else
	{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		io.MouseDown[i] = priv::mMouseJustPressed[i] || priv::mWindow->isMouseButtonPressed(i);
		priv::mMouseJustPressed[i] = false;
	}

	io.MouseWheel = priv::mMouseWheel;
	priv::mMouseWheel = 0.0f;

	// Hide OS mouse cursor if ImGui is drawing it
	priv::mWindow->setCursorMode(io.MouseDrawCursor ? GLFW_CURSOR_HIDDEN : GLFW_CURSOR_NORMAL);
...
void render()
{
	...
	glUseProgram(priv::mShaderHandle);
	glUniform1i(priv::mAttribLocationTex, 0);
	glUniformMatrix4fv(priv::mAttribLocationProjMtx, 1, GL_FALSE, &ortho_projection[0][0]);
	glBindVertexArray(priv::mVaoHandle);
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, priv::mVboHandle);
		glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)cmd_list->VtxBuffer.Size * sizeof(ImDrawVert), (const GLvoid*)cmd_list->VtxBuffer.Data, GL_STREAM_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, priv::mElementsHandle);
...
void shutdown()
{
	priv::invalidateDeviceObjects();
	ImGui::Shutdown();
}

void windowNew(const std::string& title)
{
}
